...
- (void)updateChrome {
  ...
  playButton.enabled = _photoSource.numPhotos > 1;
  _previousButton.enabled = _centerPhotoIndex > 0;
  _nextButton.enabled = _centerPhotoIndex >= 0 && _centerPhotoIndex < _photoSource.numPhotos-1;
}
...
- (void)moveToPhotoAtIndex:(NSInteger)photoIndex withDelay:(BOOL)withDelay {
  ...
  [self moveToPhoto:[_photoSource photoWithIndex:_centerPhotoIndex]];
...
- (void)updateVisiblePhotoViews {
  ...
  for (NSNumber* key in photoViews.keyEnumerator) {
    ...
    id<TTPhoto> photo = [_photoSource photoWithIndex:key.intValue];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (void)slideshowTimer {
  if (_centerPhotoIndex == _photoSource.numPhotos-1) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (BOOL)canShowModel {
  return _photoSource.numPhotos > 0;
}
...
- (void)moveToNextValidPhoto {
  if (_centerPhotoIndex >= _photoSource.numPhotos) {
    // We were positioned at an index that is past the end, so move to the last photo
    [self moveToPhotoAtIndex:_photoSource.numPhotos - 1 withDelay:NO];
...
- (UIView*)scrollView:(TTScrollView*)scrollView pageAtIndex:(NSInteger)pageIndex {
  ...
  id<TTPhoto> photo = [_photoSource photoWithIndex:pageIndex];
  [self showPhoto:photo inView:photoView];
